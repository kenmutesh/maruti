...
[
'pageTitle' => 'Aprotec | Add AP Credit Note',
'select2' => true,
]
)
<style>
  ...
  <section class="content home">
    <div class="container-fluid">
      ...
      <div class="content">
        ...
        <form class="" onsubmit="showSpinner(event)" action="{{ route('create_ap_creditnote') }}" method="post" autocomplete="off" enctype="multipart/form-data">
          @csrf
          <div class="row">
            ...
            <div class="col-sm-3">
              <div class="form-group">
                ...
                <select class="form-control searchable-select ms supplier-select" data-live-search="true" data-style="text-white" required name="supplier_id">
              ...
              <tbody class="item-list">
                <tr>
                  <td>
                    ...
                    <select style="width:200px;" data-live-search="true" data-style="text-white" class="searchable-select ms" onchange="prefillItemRow(this, true)">
                        ...
                        <?php
                          foreach ($powderInventory as $powderItem) {
                            if (session()->get('auth_warehouse_uid') == 'N/A') {
                              // an admin so shows all
                            ?>
                              <option value="<?php echo $powderItem->item_id ?>" attr-data-description="<?php echo $powderItem->powder_description ?>" attr-data-code="<?php echo $powderItem->powder_code ?>" attr-data-cost="<?php echo $powderItem->standard_cost ?>" attr-data-cost="<?php echo $powderItem->standard_cost ?>" attr-warehouse-id=<?php echo $powderItem->warehouse_id ?> attr-bin-id="<?php echo $powderItem->bin_id ?>" attr-data-name ="<?php echo $powderItem->powder_color ?>">
                                <?php echo $powderItem->powder_color ?> (<?php echo $powderItem->warehouse_name ?>)
                              </option>
                            <?php
                          }else if (session()->get('auth_warehouse_uid') == $powderItem->warehouse_id) {
                            // show only those to warehouse id subscribed to
                            ?>
                              <option value="<?php echo $powderItem->item_id ?>" attr-data-description="<?php echo $powderItem->powder_description ?>" attr-data-code="<?php echo $powderItem->powder_code ?>" attr-data-cost="<?php echo $powderItem->standard_cost ?>" attr-data-cost="<?php echo $powderItem->standard_cost ?>" attr-warehouse-id=<?php echo $powderItem->warehouse_id ?> attr-bin-id="<?php echo $powderItem->bin_id ?>" attr-data-name ="<?php echo $powderItem->powder_color ?>">
                                <?php echo $powderItem->powder_color ?> (<?php echo $powderItem->warehouse_name ?>)
                              </option>
                      ...
                      <optgroup label="ALUMINIUM">
                        <?php
                          foreach ($aluminiumInventory as $aluminiumItem) {
                            if (session()->get('auth_warehouse_uid') == 'N/A') {
                              ?>
                                <option value="<?php echo $aluminiumItem->item_id ?>" attr-data-description="<?php echo $aluminiumItem->item_description ?>" attr-data-code="<?php echo $aluminiumItem->item_code ?>" attr-data-cost="<?php echo $aluminiumItem->standard_cost ?>" attr-warehouse-id=<?php echo $aluminiumItem->warehouse_id ?> attr-bin-id="<?php echo $aluminiumItem->bin_id ?>" attr-data-name ="<?php echo $aluminiumItem->item_name ?>">
                  ...
                  <td class="p-0">
                    <input class="w-100" type="text" onkeyup="calculateItemRowTotal(this)" name="item_qty[]" value="">
                  </td>

                  <td class="p-0">
                    <input class="w-100" type="text" onkeyup="calculateItemRowTotal(this)" name="item_kg[]" value="">
                  </td>

                  <td class="p-0">
                    <input class="w-100" type="text" onkeyup="calculateItemRowTotal(this)" name="item_tax[]" value="">
                  </td>
          ...
          <button type="submit" name="submit_btn" value="Create Purchase Order" class="btn btn-success w-100 text-white">CREATE AP CREDIT NOTE</button>
