...
<body class="bg-secondary">
  ...
  <div class="row d-flex justify-content-around w-75 mx-auto">
    <button type="button" name="button" onclick="printInfo('Acquisition:{{ $acquisitionInfo->date_created }}.pdf')" class="btn btn-warning col-sm-3">
      SAVE TO LOCAL
    </button>
    ...
    <div class="dropdown">
      ...
      <div class="dropdown-menu p-4">
        ...
        <button type="button" name="button" class="btn btn-success" onclick="sendPDF(this, 'Acquisition:{{ $acquisitionInfo->date_created }}.pdf')">
          SEND DOCUMENT
        </button>
      </div>
    </div>
...
<div class="col-sm-12 mt-3 mx-auto h-100" style="width:100%;background:white;min-height:29cm;" id="documentFrame" style="background:white;font-family:'Helvetica';font-weight: lighter;">
  <h4 class="text-center">INVENTORY ACQUISITION</h4>
  ...
  <div class="row">
    <div class="col-sm-11 d-flex justify-content-between">
      ...
      <div class="col-sm-4 d-flex flex-column pt-5" style="color:#000;margin-right: -6%;">
        <span style="width:auto;font-size:14px">
          Date Created: {{ date('Y-m-d',strtotime( $acquisitionInfo->date_created )) }}
        </span>
        <span style="width:auto;font-size:14px">
          Warehouse: {{ $acquisitionInfo->warehouse->warehouse_name }}
        </span>
        <span style="width:auto;font-size:14px">
          Done by: {{ $acquisitionInfo->systemuser->username }}
        </span>
      </div>
    </div>
  </div>
  ...
  <div class="row mt-4">
    <table class="col-sm-11 mx-auto" style="border-collapse:collapse;border:1px solid #ff6600;font-size:14px;">
      <thead style="color:#000;">
        <th style="border:1px solid #ff6600;">Item Name</th>
        <th style="border:1px solid #ff6600;">Item Serial</th>
        <th style="border:1px solid #ff6600;">Acquisition Description</th>
        <th style="border:1px solid #ff6600;">Quantity Removed</th>
        <th style="border:1px solid #ff6600;">Quantity Remaining</th>
      </thead>
      <tbody>
        <?php
          $itemList = json_decode($acquisitionInfo->item_list);
        ...
        <tr style="color:#000;">
          ...
          <td style="border:1px solid #ff6600;padding:5px;">{{ $item->item_qty_remaining }}</td>
        </tr>
