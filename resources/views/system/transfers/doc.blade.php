...
<body class="bg-secondary">
  ...
  <div class="row d-flex justify-content-around w-75 mx-auto">
    <button type="button" name="button" onclick="printInfo('Transfer:{{ $transfer->date_created }}.pdf')" class="btn btn-warning col-sm-3">
      SAVE TO LOCAL
    </button>


    <div class="dropdown">
      <button type="button" class="btn btn-warning dropdown-toggle d-flex align-items-center" class="btn btn-info col-sm-3" data-toggle="dropdown">
      ...
      <div class="dropdown-menu p-4">
        ...
        <button type="button" name="button" class="btn btn-info" onclick="sendPDF(this, 'Transfer:{{ $transfer->date_created }}.pdf')">
          SEND DOCUMENT
        </button>
      </div>
    </div>
...
<div class="col-sm-12 mt-3 mx-auto h-100" style="width:100%;background:white;min-height:29cm;" id="documentFrame" style="background:white;font-family:'Helvetica';font-weight: lighter;">
  <h3 class="text-center">TRANSFER DOCUMENT</h3>
  ...
  <div class="row">
    <div class="col-sm-11 d-flex justify-content-between">
      ...
      <div class="col-sm-5 d-flex flex-column" style="color:#000;margin-right: -6%;">
        <span style="width:auto;font-size:14px">
          Date Created: {{ date('Y-m-d',strtotime( $transfer->date_created )) }}
        </span>
        <span style="width:auto;font-size:14px">
          Requested By: {{ $transfer->requester->username }}
        </span>
        <span style="width:auto;font-size:14px">
          Approved by: <?php echo $approvedBy = ($transfer->approvedby) ? $transfer->approvedby->username : 'N/A' ; ?>
        </span>
      </div>
    </div>
  </div>
    ...
    <div class="col-sm-11 mx-auto">
      <div class="col-sm-6 d-flex flex-column" style="margin-left:-1.8rem;">
        <span style="color:#000;width:auto;font-size:14px;">
          Destination Warehouse: {{ $transfer->warehouse->warehouse_name }}
        </span>
        <span style="color:#000;width:auto;font-size:14px;">
          Destination Floor: {{ $transfer->floor->floor_name ?? '-' }}
        </span>
        <span style="color:#000;width:auto;font-size:14px;">
          Destination Shelf: {{ $transfer->shelf->shelf_name ?? '-' }}
        </span>
        <span style="color:#000;width:auto;font-size:14px;">
          Destination Bin: {{ $transfer->bin->bin_name ?? '-' }}
        </span>
      </div>
    </div>
  </div>
  ...
  <div class="row mt-4">
    <table class="col-sm-11 mx-auto" style="border-collapse:collapse;border:1px solid #ff6600;font-size:14px;">
      <thead style="color:#000;">
        ...
        <th style="border-right:1px solid #ff6600;">Quantity/Weight Transferred</th>
        <th style="border-right:1px solid #ff6600;">Source Warehouse</th>
        <th style="border-right:1px solid #ff6600;">Source Bin</th>
      </thead>
      <tbody>
        <?php
          $itemList = json_decode($transfer->item_list);
        ...
        <tr style="color:#000;">
          ...
          @endif
          <td style="border:1px solid #ff6600;padding:5px;">{{ $item->source_warehouse_name }}</td>
          <td style="border:1px solid #ff6600;padding:5px;">{{ $item->source_bin_name }}</td>
        </tr>
    ...
    <div class="col-sm-11">
      <div class="col-sm-6 d-flex flex-column ml-3">
        <div class="border border-top-0 border-left-0 border-right-0 border-lg border-dark w-50 mt-4">
          <span>Approved By: Kiran K. Mavji</span>
        </div>
        <div class="border border-top-0 border-left-0 border-right-0 border-lg border-dark w-50 mt-4">
          <span>Signature:</span>
        </div>
