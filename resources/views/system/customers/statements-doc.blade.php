...
<body class="theme-green">
  ...
  <div class="row d-flex justify-content-around w-75 mx-auto">
    <button type="button" name="button" onclick="printInfo('Statement:{{ $customer->customer_name }}.pdf')" class="btn btn-info col-sm-3">
      SAVE TO LOCAL
    </button>
      ...
      <div class="dropdown-menu" style="width:25rem;left:-10rem !important;">
        <div class="col-12 p-2 email-send">
          ...
          <div class="form-group">
            ...
            <textarea class="form-control border" rows="7" name="message">Dear {{ $customer->customer_name }},
...
MARUTI GLAZERS LTD</textarea>
          ...
          <div class="form-group">
            ...
            <input type="email" name="main_email" required value="{{ $customer->contact_person_email }}" placeholder="Please insert email" class="form-control">
          </div>
          <div class="form-group cc-parent">
            <label for="">CC</label>
            @if($customer->cc_emails)
            <?php
            $ccEmails = json_decode($customer->cc_emails);
            for ($i = 0; $i < count($ccEmails); $i++) {
            ?>
              <div class="row">
                <input type="email" name="cc_emails[]" class="col-10 form-control mb-1" class="dark-text" placeholder="Enter cc person email" value="{{ $ccEmails[$i] }}">
                <button type="button" class="btn btn-sm py-0 px-1 btn-danger" onclick="removeCCEmailField(this)">
                  REMOVE
                </button>
              </div>
            <?php
          ...
          <button type="button" name="button" class="btn btn-info" onclick="sendPDF(this, 'Statement:{{ $customer->customer_name }}.pdf')">
            SEND DOCUMENT
          </button>
        </div>
        <script>
      ...
      <div class="col-sm-11 d-flex justify-content-between">
        ...
        <div class="col-sm-5 d-flex flex-column" style="color:#000;margin-right: -6%;border:1px solid #ff6600;height:fit-content;">
          <span style="width:auto;font-size:10px">
            <b>TO: {{ $customer->company }}</b>
          </span>
          <span style="width:auto;font-size:10px">
            EMAIL: {{ $customer->contact_person_email }}
          </span>
          <span style="width:auto;font-size:10px">
            TEL: {{ $customer->contact_number }}
          </span>
          <span style="width:auto;font-size:10px">
        ...
        <tfoot>
          ...
          <tr>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
              {{ number_format($customer->singleDayInvoicesAmountDue($statementDate),2) }}
            </td>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
              {{ number_format($customer->dateRangePeriodInvoicesAmountDue($statementDate, 1, 30),2) }}
            </td>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
              {{ number_format($customer->dateRangePeriodInvoicesAmountDue($statementDate, 31, 61),2) }}
            </td>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
              {{ number_format($customer->dateRangePeriodInvoicesAmountDue($statementDate, 61, 91),2) }}
            </td>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
              {{ number_format($customer->singlePeriodOverInvoicesAmountDue($statementDate, 91),2) }}
            </td>
            <td style="border:1px solid #ff6600;color: #000;" class="text-center">
