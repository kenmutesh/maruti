                    ...
                    <div class="col-sm-4 text-center">
                      <div class="dropdown">
                        ...
                        <div class="dropdown-menu">
                          @forelse ($allCashSales as $singleCashSaleBatch)
                          <li class="dropdown-item d-flex align-items-baseline">
                            <?php $cashSaleSection = ''; ?>
                            @foreach ($singleCashSaleBatch as $singleCashSale)
                            @if ($loop->first)
                            <?php $cashSaleSection .= '/' . $singleCashSale->cash_sale_suffix; ?>
                            {{ $singleCashSale->cash_sale_prefix }}{{ $singleCashSale->cash_sale_suffix }} -
                            @elseif($loop->last)
                            <?php $cashSaleSection .= '/' . $singleCashSale->cash_sale_suffix; ?>
                            {{ $singleCashSale->cash_sale_prefix }}{{ $singleCashSale->cash_sale_suffix }}
                            @endif
                            @endforeach
                            <a href="/cashsales/sections{{ $cashSaleSection }}" class="btn btn-primary btn-sm py-0 px-2">View</a>
                          </li>
                          @empty
                          <li><a class="dropdown-item" href="#">No cash sales present</a></li>
                          @endforelse
                        </div>
                      </div>
                    </div>
